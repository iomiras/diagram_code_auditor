...
with Diagram("Class Relationships and Methods", direction="TB", show=False, graph_attr=graph_attr):
    ...
    with Cluster("Extra Cluster"):
        extra_class = Server("ExtraClass")
        extra_class >> Edge(label="connects_to()") >> user

    # Adds a method not implemented in the code
    user >> Edge(label="forgot_password()") >> user
