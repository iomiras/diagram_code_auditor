...
#     """
#     if isinstance(node, ast.Call) and not node.func.id == "Edge":
#         return next(
...
#     print("Diagram class to methods", class_to_methods)
#
...
#     print ("Code class to methods", class_to_methods)
...
#     # Parse arguments
#     code_file_name = "classes/test_class.py"
#     diagram_file_names = ["diagrams/test_diagram.py"]
...
def extract_method_from_edge(node):
    ...
    # Ensure the called function is Edge
    ...
    for kw in node.keywords:
        if kw.arg == "label":
            if isinstance(kw.value, ast.Constant):
                ...
                # Label given but not as a direct string constant
                # Could add logic here to resolve variable references if needed
...
class DiagramVisitor(ast.NodeVisitor):
    ...
    from a diagram definition using the `diagrams` library.
    """
    ...
    def _handle_list_assignment(self, variable, value):
        ...
        # where classVar1, etc., are variables holding class definitions
    ...
    def visit_BinOp(self, node):
        ...
        E.g. base >> Edge(label="method") >> child
        """
        try:
    ...
    def visit_For(self, node):
        ...
        E.g. for x in [classVar1, classVar2]: x >> Edge(...) >> [classVar3]
        """
        try:
    ...
    def add_to_connections(self, left_class_ids, method, right_class_ids, op):
        ...
        for left_id in left_class_ids:
            ...
            for right_id in right_class_ids:
                ...
                # Depending on the operator, connection direction may differ
                if isinstance(op, ast.RShift):
        ...
        for class_name, parents in self.class_to_parents.items():
            for parent in parents:
                if parent not in resolved_methods:
                    # Parent not found in local classes (external?). Just skip.
                    continue
                resolved_methods[class_name].update(resolved_methods[parent])
