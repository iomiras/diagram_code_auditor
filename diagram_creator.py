...
def write_diagram(file_path, diagram_name, classes, class_to_methods, connections):
    ...
    with open(file_path, 'w') as f:
        ...
        f.write(f"with Diagram(\"{diagram_name}\", filename= \"./{file_path.split('/')[-1]}\", direction=\"TB\", show=False, graph_attr=graph_attr):\n")
        ...
        for conn in connections:
            ...
            if len(to_classes) > 1:
                ...
                f.write(f"    {from_cls.lower()} >> Edge(label=\"{method}\", style='dotted', color='gray') >> [{to_vars}]\n")
            else:
        ...
        for cls, methods in class_to_methods.items():
            for method in methods:
                if (cls, method) not in connected_methods:
                    f.write(f"    {cls.lower()} >> Edge(label=\"{method}\", style='dotted', color='blue') >> {cls.lower()}\n")
    f.close()
